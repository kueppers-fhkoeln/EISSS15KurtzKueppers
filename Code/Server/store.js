...
var store = {
    ...
    saveGPS: function(gps_data, callback) {
        if (gps_data == null){
            ...
            Fahrer.findOne({pid:gps_data.pid, eid:gps_data.eid}, function(err, eintrag){
                if (eintrag == null){
                    Fahrer.save(gps_data, callback);
                }else if (eintrag.pid == gps_data.pid && eintrag.eid == gps_data.eid){ 
                    if (eintrag.longitude != gps_data.longitude && eintrag.latitude != gps_data.latitude)
                        //ToDO: Sende Daten an den Fahrer
                        Fahrer.update(gps_data, callback);
                }else{
                    Mitfahrer.findOne({pid:gps_data.pid, eid:gps_data.eid}, function(err, eintrag){
                        if (eintrag == null){
                            Fahrer.save(gps_data, callback);
                        }else if (eintrag.pid == gps_data.pid && eintrag.eid == gps_data.eid){ 
                                    Mitfahrer.update(gps_data, callback);
                        }else{
                            callback("GPS Daten konnten keiner Person zugeordnet werden!");
                        }
                    });
                }
            });      
...
//----------------------------------------------------------------------------------------------------------------------------------------------
//Weitere Funktionen/ Variablen vom Server ----------------------------------------------------------------------------------------------------------------
var datum = new Date();
